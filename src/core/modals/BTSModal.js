...
export const createBTSModal = (api, createMainContainer) => {
    ...
    const { hide, hideBTS, acceptCategory } = api;
   ...
   */
    const modalData =
    state._currentTranslation && state._currentTranslation.BTSModal;
     ...
     */
        addEvent(pmOverlay, CLICK_EVENT, hideBTS);
        ...
        addEvent(dom._pmCloseBtn, CLICK_EVENT, hideBTS);
    ...
    if (!state._BTSModalExists) {
        state._BTSModalExists = true;

        debug('CookieConsent [HTML] created', BTS_MODAL_NAME);

        fireEvent(globalObj._customEvents._onModalReady, BTS_MODAL_NAME, dom._pm);
